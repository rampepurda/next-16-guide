...
export default async function PokemonPage({
  ...
  const { data, error, loading } = useQuery(GET_POKEMON_ABILITY_V2, {
    variables: {
      pokemonV2AbilityByPkId: Number(pokeId),
    },
  })

  console.log(data?.pokemon_v2_ability_by_pk.name)

  return <>{data?.pokemon_v2_ability_by_pk && <h2>{data.pokemon_v2_ability_by_pk.name}</h2>}</>
}
